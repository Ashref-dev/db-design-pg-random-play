...
BEGIN
    ...
    SELECT pkg_customers.add_customer(
        'RentalTest', 'Customer', 'rental.test@example.com', 
        '555-RENTAL', '888 Rental Rd, Testville'
    ) INTO v_customer_id;
    
    -- Get a tape ID to rent
    ...
    IF v_tape_id IS NULL THEN
        RAISE EXCEPTION 'Test C7 FAILED: Test setup issue - no tapes available for rental';
    END IF;
    ...
    -- Now try to delete the customer - this should fail
    BEGIN
        SELECT pkg_customers.delete_customer(v_customer_id) INTO v_success;
    ...
    -- Clean up - return the tape and delete the customer
    UPDATE rentals
    SET return_date = CURRENT_TIMESTAMP
    WHERE rental_id = v_rental_id;
    
    -- Update the tape stock back
    UPDATE tapes
    SET stock_available = stock_available + 1
    WHERE tape_id = v_tape_id;
    
    -- Now we should be able to delete the customer
